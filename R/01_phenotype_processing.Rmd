...
phenotype <- read.csv(
  file = "Alldata2016-2018 3sd NA_FINAL_NO_ISSUES.csv") 
```
...
ggsave(filename = "BLUEs_PIC_output/top_median_yield.pdf", 
...
ggsave(filename = "BLUEs_PIC_output/bottom_median_yield.pdf", 
       plot = p1, device = pdf, width = 33, height = 20) 
```
...
```{r, message=FALSE}
pdf(file = "BLUEs_PIC_output/yield_qq.pdf")
...
ggsave(filename = "BLUEs_PIC_output/top_median_Max_Max_height_m.pdf", 
       plot = p1, device = pdf, width = 33, height = 20) 
```
...
ggsave(filename = "BLUEs_PIC_output/bottom_median_Max_Max_height_m.pdf", 
       plot = p1, device = pdf, width = 33, height = 20) 
```
...
```{r, message=FALSE}
pdf(file = "BLUEs_PIC_output/Max_Max_height_m_qq.pdf")
...
ggsave(filename = "BLUEs_PIC_output/top_median_Max_Ave_height_m.pdf", 
       plot = p1, device = pdf, width = 33, height = 20) 
```
...
ggsave(filename = "BLUEs_PIC_output/bottom_median_Max_Ave_height_m.pdf", 
       plot = p1, device = pdf, width = 33, height = 20) 
```
...
```{r, message=FALSE}
pdf(file = "BLUEs_PIC_output/Max_Ave_height_m_qq.pdf")
...
ggsave(filename = "BLUEs_PIC_output/top_median_Max_area_plant_m2.pdf", 
       plot = p1, device = pdf, width = 33, height = 20) 
```
...
ggsave(filename = "BLUEs_PIC_output/bottom_median_Max_area_plant_m2.pdf", 
       plot = p1, device = pdf, width = 33, height = 20) 
```
### Calculate BLUEs
...
```{r, message=FALSE}
pdf(file = "BLUEs_PIC_output/Max_area_plant_m2_qq.pdf")
...
ggsave(filename = "BLUEs_PIC_output/top_median_Max_volume_plant_m3.pdf", 
       plot = p1, device = pdf, width = 33, height = 20) 
```
...
ggsave(filename = "BLUEs_PIC_output/bottom_median_Max_volume_plant_m3.pdf", 
       plot = p1, device = pdf, width = 33, height = 20) 
```
...
```{r, message=FALSE}
pdf(file = "BLUEs_PIC_output/Max_volume_plant_m3_qq.pdf")
...
# Perform ANOVA (this runs without graphics issues)
anova(model)

# Open a PDF device to save the plots
pdf(file = "BLUEs_PIC_output/yield_vs_max_height_diagnostics.pdf")

# Generate QQ-norm plot of residuals
qqnorm(residuals(model))
...
# Close the PDF device, saving all plots
dev.off()
```
...
DAPC_Q_matrix <- read.csv(file = "DAPC_PIC_output/DAPC_membership_probabilities.csv") %>%
...
STRUCTURE_Q_matrix <- read.table("STRUCTURE/K3.outfile")
...
PCs <- read.csv(file = "DAPC_PIC_output/PCA_principal_components.csv") %>%
...
write.csv(BLUEs_with_Q_matrices_and_PCs, 
          "BLUEs_PIC_output/BLUEs_with_Q_matrices_and_PCs.csv",
          row.names = FALSE)

```
```{r}
list.files("STRUCTURE/final_results/bestK/CLUMPP/")
```


# References

BATES, D., MÃ„CHLER, M., BOLKER, B., and WALKER, S. (2015). â€˜Fitting Linear Mixed-Effects Models Using lme4,â€™ Journal of Statistical Software, 67(1), pp. 1 - 48. doi:10.18637/jss.v067.i01.

YANG, C.J., RUSSELL, J., RAMSAY, L., THOMAS, W., POWELL, W., and MACKAY, I. (2021). â€˜Overcoming barriers to the registration of new plant varieties under the DUS system,â€™ Communications Biology, 4(1), pp. 302. doi:10.1038/s42003-021-01840-9.
