...
gene_dosage <- read.csv(file = "sharma_genotype_reduced_recoded.csv")
...
```{r}
#load SNP positions
SNP_pos <- read_excel(path = "sharma_SNP_positions.xlsx", sheet = "annotation")
...
```{r, eval=FALSE}
dir.create("DAPC_PIC_output/", showWarnings = FALSE)
#set up tiff to save plot
tiff(file = "DAPC_PIC_output/glPlot.tiff", height = 1500, width = 1500, res = 100)
...
pdf(file = "DAPC_PIC_output/SNP_dist.pdf", width = 23, height = 20)
...
```{r}
#set up pdf to save plot
pdf(file = "DAPC_PIC_output/PCA_scree.pdf")
...
```{r}
#set up pdf to save plot
pdf(file = "DAPC_PIC_output/PCA_scatter.pdf")
...
```{r}
#set up pdf to save plot
pdf(file = "DAPC_PIC_output/PCA_loadings.pdf")
...
write.csv(all_PC_loadings_0.0025, file = "DAPC_PIC_output/all_PC_loadings_0.0025.csv",
          row.names = FALSE)
```
...
```{r}
#set up pdf to save plot
pdf(file = "DAPC_PIC_output/SNP_dist_PIC_filter.pdf", width = 23, height = 20)
...
pdf(file = "DAPC_PIC_output/kmeans_clustering.pdf")
...
```{r, eval=FALSE}
#set up pdf to save plots
pdf(file = "DAPC_PIC_output/initial_dapc.pdf")
...
```{r, eval=FALSE}
#set up pdf to save plots
pdf(file = "DAPC_PIC_output/DAPC_optim_a_score.pdf")
...
```{r, eval=FALSE}
#set up pdf to save plots
pdf(file = "DAPC_PIC_output/DAPC_optim_PCA.pdf")
...
```{r, eval=FALSE}
#set up pdf to save plots
pdf(file = "DAPC_PIC_output/DAPC_optim_PCA_PC5v6.pdf")
...
```{r, eval=FALSE}
#set up pdf to save plots
pdf(file = "DAPC_PIC_output/final_dapc.pdf")
...
```{r, eval=FALSE}
#set up pdf to save plots
pdf(file = "DAPC_PIC_output/dapc_scatter.pdf", width = 10)
...
rbind(prior, posterior) %>%
  ...
  write.csv(., file = "DAPC_PIC_output/group_assignments.csv", row.names = FALSE)
...
```{r, eval=FALSE}
#set up pdf to save plots
pdf(file = "DAPC_PIC_output/dapc_discriminant_plots.pdf")
...
```{r, eval=FALSE}
#set up pdf to save plots
pdf(file = "DAPC_PIC_output/dapc_loadings.pdf")
...
write.csv(all_LD_loadings_0.005, file = "DAPC_PIC_output/all_LD_loadings_0.005.csv",
          row.names = FALSE)
```
...
ggsave(filename = "DAPC_PIC_output/cluster_membership_graph_cultivar_names.pdf", p1, width = 40, height = 20)
```
...
ggsave(filename = "DAPC_PIC_output/cluster_membership_graph_numeric_IDs.pdf", p2, width = 30, height = 10)
```
...
ggsave(filename = "DAPC_PIC_output/most_likely_clusters.pdf", p3, width = 30, height = 10)
```
...
as.data.frame(final_dapc[["posterior"]]) %>%
  rownames_to_column(var = "ID") %>%
  write.csv(., file = "DAPC_PIC_output/DAPC_membership_probabilities.csv", row.names = FALSE)
```
...
as.data.frame(PCA_6PC[["scores"]][,1:5]) %>%
  as.data.frame() %>%
  rownames_to_column(var = "ID") %>%
  write.csv(., file = "DAPC_PIC_output/PCA_principal_components.csv", row.names = FALSE)
```

# References

JOMBART, T. (2008). â€˜adegenet: a R package for the multivariate analysis of genetic markers,â€™ Bioinformatics, 24(11), pp. 1403-5. <doi:10.1093/bioinformatics/btn129>.

JOMBART, T., and AHMED, I. (2011). â€˜adegenet 1.3-1: new tools for the analysis of genome-wide SNP data,â€™ Bioinformatics, 27(21), pp. 3070-1. <doi:10.1093/bioinformatics/btr521>.

THIA, J.A. (2023). â€˜Guidelines for standardizing the application of discriminant analysis of principal components to genotype data,â€™ Molecular Ecology Resources, 23(3), pp. 523-538. <doi:10.1111/1755-0998.13706>.

SHARMA, S.K., MACKENZIE, K., MCLEAN, K., DALE, F., DANIELS, S., and BRYAN, G.J. (2018). â€˜Linkage Disequilibrium and Evaluation of Genome-Wide Association Mapping Models in Tetraploid Potato,â€™ G3 (Bethesda), 8(10), pp. 3185-3202. <doi:10.1534/g3.118.200377>.
